...
public abstract class RSAUtils {
	...
	private static String getRSAPairFilePath() {
		...
		URL url = RSAUtils.class.getClassLoader().getResource(RSA_PAIR_FILENAME.substring(1));
		LOGGER.error("当前获取到的RSA文件路径为：" + url.getPath());
		return url.getPath();
...
//			ex.printStackTrace();
	...
	private static KeyPair readKeyPair() {
		LOGGER.error("同步读出保存的密钥对");
		FileInputStream fis = null;
		ObjectInputStream ois = null;
		try {
			LOGGER.error("同步读出保存的密钥对:" + rsaPairFile);
			fis = FileUtils.openInputStream(rsaPairFile);
			// 增加反序列化的白名单，预防安全问题 linbx 20160906
			ois = new ValidatingObjectInputStream(fis).accept("org.bouncycastle.jcajce.provider.asymmetric.rsa.*")
					.accept("java.security.KeyPair").accept("java.*").accept("[B");
			oneKeyPair = (KeyPair) ois.readObject();
			LOGGER.error("同步读出保存的密钥对成功对象地址为:" + oneKeyPair == null ? ""
					: oneKeyPair.getPrivate() + "     " + oneKeyPair.getPublic());
			return oneKeyPair;
		} catch (IOException e) {
	...
	public static RSAPrivateKey generateRSAPrivateKey(byte[] modulus, byte[] privateExponent) {
		...
		try {
			 ...
			 * catch (NullPointerException ex) { LOGGER.error(
			 * "RSAUtils#KEY_FACTORY is null, can not generate KeyFactory instance.", ex); }
			 */
		return null;
	}
	 ...
	 * @throws NoSuchAlgorithmException
	 */
	public static byte[] decrypt(PrivateKey privateKey, byte[] data) throws IllegalBlockSizeException,
	...
	public static String decryptString(PrivateKey privateKey, String encrypttext) {
		...
		try {
			...
			LOGGER.error(String.format("\"%s\" Decryption failed. Cause: %s", encrypttext, e.getCause().getMessage()));
		} catch (IllegalBlockSizeException e) {
			LOGGER.error(String.format("\"%s\" Decryption failed. Cause: %s", encrypttext, e.getCause().getMessage()));
		} catch (BadPaddingException e) {
			LOGGER.error(String.format("\"%s\" Decryption failed. Cause: %s", encrypttext, e.getCause().getMessage()));
	...
	public static String decryptString(String encrypttext) {
		...
		try {
			...
			LOGGER.error(new Date() + "Decryption failed", e);
		} catch (InvalidKeyException e) {
			LOGGER.error(new Date() + "Decryption failed", e);
		} catch (IllegalBlockSizeException e) {
			LOGGER.error(new Date() + "Decryption failed", e);
	...
	public static PrivateKeyMap getPrivateKeyMap() {
		PrivateKeyMap privateKeyMap = new PrivateKeyMap();
		RSAPrivateKey rsaPrivateKey = getDefaultPrivateKey();
		privateKeyMap.setModulus(new String(Hex.encodeHex(rsaPrivateKey.getModulus().toByteArray())));
		privateKeyMap.setExponent(new String(Hex.encodeHex(rsaPrivateKey.getPrivateExponent().toByteArray())));
